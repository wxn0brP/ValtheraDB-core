...
export function hasFieldsAdvanced(obj: Object, fields: Arg) {
	...
	// Subset check
	if ("$subset" in $fields && !hasFields(obj, $fields["$subset"])) return false;
	...
	if ("$and" in $fields) {
		return $fields["$and"].every((subFields: Object) =>
			hasFieldsAdvanced(obj, subFields),
		);
	}

	if ("$or" in $fields) {
		return $fields["$or"].some((subFields: Object) =>
			hasFieldsAdvanced(obj, subFields),
		);
	}
...
function mainCheck(obj: Object, fields: Object) {
	return _for(fields, obj, {
		...
		arrinc: (data, values) =>
			Array.isArray(data) && values.some((val: any) => data.includes(val)),
		arrincall: (data, values) =>
			Array.isArray(data) && values.every((val: any) => data.includes(val)),
...
function _for(
	...
	opts: Record<string, (data: any, value: any, key: string) => boolean>,
): boolean {
