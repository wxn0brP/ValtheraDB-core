...
import { Executor, SmartExecutor } from "../helpers/executor.js";
...
export class ValtheraClass {
    ...
    async execute(name, query) {
        await this.init();
        const result = await this.executor.addOp(this.dbAction[name].bind(this.dbAction), query, typeof query === "string" ? query : query.collection);
