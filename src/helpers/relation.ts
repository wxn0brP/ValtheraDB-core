...
export function pickByPath(obj: any, paths: string[][]): any {
	const result = {};
...
export function autoSelect(
	rel: RelationTypes.RelationConfig,
	key: string,
): [
	string[] | undefined,
	boolean,
] {
	const select = Array.isArray(rel.select)
		? [
				...rel.select,
			]
		: undefined;
...
export function convertSearchObjToSearchArray(
	...
	return Object.entries(obj).reduce((acc, [key, value]) => {
		...
		if (!value) {
			...
			return [
				...acc,
				...convertSearchObjToSearchArray(value, currentPath),
			];
		} else {
			return [
				...acc,
...
export async function processRelations(
	...
	for (const key in cfg) {
		...
		if (type === "1") {
			...
			for (const item of targets) item[as] = grouped[item[pk]] || [];

			if (rel.relations)
				await Promise.all(
					results.map(row => processRelations(dbs, rel.relations!, row)),
				);
