...
describe("hasFieldsAdvanced", () => {
	...
	test("2. should throw error for non-object fields", () => {
		expect(() => hasFieldsAdvanced({}, null)).toThrow(
			"Fields must be an object",
		);
		// @ts-expect-error
		expect(() => hasFieldsAdvanced({}, "string")).toThrow(
			"Fields must be an object",
		);
		// @ts-expect-error
		expect(() => hasFieldsAdvanced({}, 123)).toThrow(
			"Fields must be an object",
		);
	});
	...
	test("15. should handle $startsWith operator", () => {
		const obj = { text: "hello world" };
		expect(hasFieldsAdvanced(obj, { $startsWith: { text: "hello" } })).toBe(
			true,
		);
		expect(hasFieldsAdvanced(obj, { $startsWith: { text: "world" } })).toBe(
			false,
		);
	});
	...
	test("16. should handle $endsWith operator", () => {
		...
		expect(hasFieldsAdvanced(obj, { $endsWith: { text: "hello" } })).toBe(
			false,
		);
	});
	...
	test("24. should handle deep checks for nested object comparisons", () => {
		...
		// Test deep equality
		expect(hasFieldsAdvanced(obj, { user: { profile: { age: 25 } } })).toBe(
			true,
		);
		expect(hasFieldsAdvanced(obj, { user: { profile: { age: 30 } } })).toBe(
			false,
		);
	...
	test("25. should handle deep operator checks with nested objects", () => {
		...
		expect(hasFieldsAdvanced(obj, { $gte: { a: { b: { c: 10 } } } })).toBe(
			true,
		);
		expect(hasFieldsAdvanced(obj, { $lte: { a: { b: { c: 10 } } } })).toBe(
			true,
		);
		expect(
			hasFieldsAdvanced(obj, { $in: { a: { b: { c: [8, 9, 10] } } } }),
		).toBe(true);
		expect(
			hasFieldsAdvanced(obj, { $nin: { a: { b: { c: [1, 2, 3] } } } }),
		).toBe(true);
		expect(
			hasFieldsAdvanced(obj, { $type: { a: { b: { c: "number" } } } }),
		).toBe(true);
	});
	...
	test("27. should handle deep checks with $regex operator and nested objects", () => {
		...
		expect(
			hasFieldsAdvanced(obj, {
				$regex: { user: { profile: { email: "example" } } },
			}),
		).toBe(true);
		expect(
			hasFieldsAdvanced(obj, {
				$regex: { user: { profile: { email: "^john" } } },
			}),
		).toBe(true);
		expect(
			hasFieldsAdvanced(obj, {
				$regex: { user: { profile: { description: "developer" } } },
			}),
		).toBe(true);
		expect(
			hasFieldsAdvanced(obj, {
				$regex: { user: { profile: { email: "test" } } },
			}),
		).toBe(false);
	});
	...
	test("28. should handle deep checks with $size operator and nested objects", () => {
		...
		expect(hasFieldsAdvanced(obj, { $size: { data: { items: 3 } } })).toBe(
			true,
		);
		expect(hasFieldsAdvanced(obj, { $size: { data: { text: 5 } } })).toBe(true);
		expect(hasFieldsAdvanced(obj, { $size: { data: { items: 2 } } })).toBe(
			false,
		);
	});
	...
	test("29. should handle deep checks with $not operator and nested objects", () => {
		...
		expect(hasFieldsAdvanced(obj, { $not: { a: { b: { c: 15 } } } })).toBe(
			true,
		);
		expect(hasFieldsAdvanced(obj, { $not: { a: { b: { c: 10 } } } })).toBe(
			false,
		);
		expect(
			hasFieldsAdvanced(obj, { $not: { $gt: { a: { b: { c: 15 } } } } }),
		).toBe(true);
		expect(
			hasFieldsAdvanced(obj, { $not: { $gt: { a: { b: { c: 5 } } } } }),
		).toBe(false);
	});
	...
	test("30. should handle deep checks with complex nested objects and operators", () => {
		...
		expect(
			hasFieldsAdvanced(obj, {
				$gt: { level1: { level2: { level3: { value: 40 } } } },
			}),
		).toBe(true);
		expect(
			hasFieldsAdvanced(obj, {
				$size: { level1: { level2: { level3: { items: 3 } } } },
			}),
		).toBe(true);
		expect(
			hasFieldsAdvanced(obj, {
				level1: { level2: { level3: { nested: { deep: true } } } },
			}),
		).toBe(true);
	...
	test("31. should handle deep checks with null and undefined values in nested structures", () => {
		...
		expect(hasFieldsAdvanced(obj, { a: { b: { c: null, e: "exists" } } })).toBe(
			true,
		);
	...
	test("32. should handle deep checks with $between operator and nested objects", () => {
		...
		expect(
			hasFieldsAdvanced(obj, { $between: { data: { score: [80, 90] } } }),
		).toBe(true);
		expect(
			hasFieldsAdvanced(obj, { $between: { data: { score: [90, 100] } } }),
		).toBe(false);
	});
	...
	test("33. should handle deep checks with $startsWith and $endsWith operators and nested objects", () => {
		...
		expect(
			hasFieldsAdvanced(obj, { $startsWith: { user: { name: "John" } } }),
		).toBe(true);
		expect(
			hasFieldsAdvanced(obj, { $endsWith: { user: { name: "Doe" } } }),
		).toBe(true);
		expect(
			hasFieldsAdvanced(obj, { $startsWith: { user: { name: "Jane" } } }),
		).toBe(false);
		expect(
			hasFieldsAdvanced(obj, { $endsWith: { user: { name: "Smith" } } }),
		).toBe(false);
	});
