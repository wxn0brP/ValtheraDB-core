...
describe("createMemoryValthera - collection operations", () => {
	test("11. should allow updateOne to update a single document", async () => {
		...
		const john = await db.users.findOne({
			name: "John",
		});
		expect(john?.age).toBe(35);

		const jane = await db.users.findOne({
			name: "Jane",
		});
	...
	test("13. should allow removeOne to remove a single document", async () => {
		...
		const result = await db.users.removeOne({
			name: "Jane",
		});
	...
	test("15. should update an existing document with updateOneOrAdd", async () => {
		...
		const result = await db.users.updateOneOrAdd(
			{
				name: "John",
			},
			{
	...
	test("17. should remove an existing document with toggleOne", async () => {
		...
		const result = await db.users.toggleOne(
			{
				name: "John",
			},
			{},
