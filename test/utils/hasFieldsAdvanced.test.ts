...
describe("hasFieldsAdvanced", () => {
	...
	])("should handle comparison operators ($#)", ({ obj, filter, expected }) => {
		expect(hasFieldsAdvanced(obj, filter)).toBe(expected);
	});
	...
	test.each([
		...
		"should handle collection/string/advanced operators ($#)",
		({ obj, filter, expected }) => {
			expect(hasFieldsAdvanced(obj, filter)).toBe(expected);
		},
	);
	...
	test("26. should handle deep checks with $exists operator - but note limitations", () => {
		...
		).toBe(false); // 'a' exists but should not
		expect(
			hasFieldsAdvanced(obj, {
				$exists: {
					z: false,
		...
		).toBe(true); // 'z' doesn't exist and shouldn't
	});
	...
	test.each([
		...
		"should handle $ne and case-insensitive operators ($#)",
		({ obj, filter, expected }) => {
			expect(hasFieldsAdvanced(obj, filter)).toBe(expected);
		},
	);
	...
	test("40. should handle $idGt operator", () => {
		const obj = {
			_id: "1gc1303x-2",
		};
		expect(
	...
	test("41. should handle $idLt operator", () => {
		const obj = {
			_id: "1gc1302w-1",
		};
		expect(
	...
	test("42. should handle $idGte operator", () => {
		const obj = {
			_id: "1gc1302w-1",
		};
		expect(
	...
	test("43. should handle $idLte operator", () => {
		const obj = {
			_id: "1gc1302w-1",
		};
		expect(
