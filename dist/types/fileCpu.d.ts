...
export interface FileCpu {
     ...
     */
    find(file: string, config: Query.FindQuery): Promise<DataInternal[]>;
    /**
     * Asynchronously finds one entry in a file based on search criteria.
     * @returns A promise resolving to the found entry or `false` if not found.
     */
    findOne(file: string, config: Query.FindOneQuery): Promise<DataInternal | false>;
    /**
   ...
   */
    remove(file: string, config: Query.RemoveQuery, one: boolean): Promise<DataInternal[]>;
    /**
     ...
     */
    update(file: string, config: Query.UpdateQuery, one: boolean): Promise<DataInternal[]>;
}
