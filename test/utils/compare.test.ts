...
describe("compareSafe", () => {
	...
	test("2. should compare strings correctly", () => {
	...
	test("3. should compare numbers correctly", () => {
	...
	test("4. should compare dates correctly", () => {
	...
	test("5. should compare number with date correctly", () => {
		const timestamp = 1672531200000; // Jan 1, 2023 timestamp
		const date = new Date(timestamp);
		expect(compareSafe(timestamp, date)).toBe(0);
		expect(compareSafe(timestamp, new Date(timestamp + 86400000))).toBeLessThan(
			0,
		); // 86400000ms = 1 day
	});

	test("6. should compare booleans correctly", () => {
