...
export class MultiBackend extends ActionsBase {
    ...
    async add(config) {
        return await this.primaryBackend.add({
            ...config,
        });
    }
    async find(config) {
        const results = await Promise.all(this.backends.map(b => b.find({
            ...config,
        })));
        return results.flat();
    }
    ...
    async update(config) {
        const results = await Promise.all(this.backends.map(b => b.update({
            ...config,
        })));
        return results.flat();
    }
    ...
    async remove(config) {
        const results = await Promise.all(this.backends.map(b => b.remove({
            ...config,
        })));
        return results.flat();
    }
    async removeOne(config) {
        for (const backend of this.backends) {
            try {
                const result = await backend.removeOne({
                    ...config,
                });
                if (result)
                    return result;
