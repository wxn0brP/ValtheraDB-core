...
export class RoutedStorage extends ActionsBase {
    ...
    constructor(rules, defaultBackend) {
        super();
        this.rules = rules;
        this.defaultBackend = Array.isArray(defaultBackend)
            ? defaultBackend
            : [
                defaultBackend,
            ];
    ...
    _matchBackends(config) {
        ...
        for (const rule of this.rules) {
            ...
            if ((typeof match === "string" && match === collection) ||
                match === "*" ||
    ...
    async getCollections() {
        const all = await Promise.all(Array.from(new Set(this.rules.flatMap(r => r.backends))).map(b => b.getCollections()));
